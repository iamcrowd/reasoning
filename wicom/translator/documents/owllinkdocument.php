...
class OWLlinkDocument extends Document{
     ...
     */
    public function start_document($ontologyIRI = null, $req = []){
    ...
    /**
       ...
       @param $uri {ntring} (Optional). The name or the URI of the
       database to release. If not given, use the actual_kb one.
     */
    public function insert_release_kb($iri=null){
	if ($iri == null){
	    $iri = $this->actual_kb;
	}

	$this->content->startElement("ReleaseKB");
	$this->content->writeAttribute("kb", $iri);
	$this->content->endElement();

	// $iri can be given by parameter or setted by this methods
	// when $iri is null (or not given).
	// Whenever be the case, it should be checked if it is the
	// same as actual_kb.
	if ($iri == $this->actual_kb) {
	    $this->actual_kb = null;
	}
